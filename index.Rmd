...
* A custom python script then processess the intersected BED file to produce the final output, either counts of different types of annotations, or length proportions of each of types of annotations. Currently the following types of annotations are recorded: pseudogene, CDS, UTR, intron, lnc_RNA, other (any other type), unannotated (or intergenic).

Slurm submission script: ```download_reference.sh```

* Download reference genome (FASTA) and annotation (GFF) for Pan troglodytes (chimpanzee) [GCF_002880755.1](https://www.ncbi.nlm.nih.gov/data-hub/genome/GCF_002880755.1/)
```{bash, comment=NA, echo=FALSE, class.output="bg_slurm"}
...
* adjust file paths
* adjust the command options on line 30 for the last script (see below)
```{bash, comment=NA, echo=FALSE, class.output="bg_slurm"}
cat annotation/annotation_job.sh
```

Slurm submission script: ```annotation_getTaxContigs.py```

* Python script run by previous shell script
...
Slurm submission script: ```annotation_blast_parser.py```

* BLAST to BED script run by the previous shell script
* adjust parameters on lines 10-12 as needed
```{bash, comment=NA, echo=FALSE, class.output="bg_python"}
cat annotation/annotation_blast_parser.py
```

Slurm submission script: ```annotation_bed2table.py```

* BED to table script run by the previous shell script
